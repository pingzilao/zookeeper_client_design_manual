...
----------------------------------华丽分割线----------------------------------------------
2. 根据zookeeper和官方C库的特点，提出如下注意点：
a) clientid只有在收到事件ZOO_CONNECTED_STATE才能获取成功，并且只在会话有效期内可用。
   一旦会话失效的，要想利用clientid重建会话，不可行。
b) zk的临时节点以及订阅请求 归属于会话，一旦会话失效，将无效。
   zk只能完成单次订阅，需要再次订阅必须再次请求。
c) zk的会话超时检测发生在服务器端。客户端只有在重连上服务器时，才被告知会话超时，并强制关闭连接。
d) 会话失效后zhandle必须调用zookeeper_close释放资源，防止资源泄漏。并且使用已关闭的zhandle会报错。
   zhandle存在并发问题。如果想在回调线程中处理重连问题，调用线程和回调线程需要加锁(会话超时时会触发回调函数，依次调用是zookeeper_close，zookeeper_init，并重新得到新的zhandle）。
e) 由于zookeeper_init是异步的，所以返回ZOK并不代表连接成功。
...
g) 两次会话之间（老会话创建，新会话还未创建），数据发生变化(客户端在订阅了数据变化的操作后会话超时，此时若节点数据变化了，在客户端重现建立新会话并且重新订阅watch后，客户端无法感知该数据变化)。
h) zk不支持持续订阅。
...
i) zk集群节点越多，同步时间越长，写性能越差。zk连接数越多，心跳检测即会话检测越多，性能越差。observer可以解决后者。
l) zk订阅子节点变化，在收到子节点变化，获取子节点列表，获取子节点数据之间，存在不一致问题，尤其是获取子节点列表成功，而获取子节点内容失败，此时不会再次通知。
m) 回调分成sessionWatcher和otherWatcher两种，前者主要收到state事件，后者收到seesion_event和type事件。
...
p) 对node和children的订阅操作以及临时节点的创建操作，一旦成功就应该在map中保存Node(path, context), Children((path, context)，(tmpNode)。若会话超时后“恢复会话时”（即收到connected事件后），重新订阅并建临时节点。
   这里的临时节点是指 第一种用途。
q) 持续订阅过程中，可能会出现操作失败问题。失败操作保存失败map中，定时再操作。当收到seesion_expired事件时清空失败map。
r) context是void*类型，由上层应用负责释放(回调函数中)，若是碰到归属不明确的，应该用shared_ptr
s) 响应时间:socket连接 < zk会话时间(临时节点) < 缓存(如果配置了)。
   server1发现与server2之间的socket连接断开后，server1重新去zk上获取可用服务列表，依然有可能获取到server2.
   所以在发现连接断开后，直接去取相应子节点，可能会取到原先的节点(节点挂了，但是会话未超时)
t) zoo_get接口，数据为空时，buffer_len参数时而返回0，时而返回-1。官方文档上写返回-1。这个得统一处理，不能直接返回给上层。
u) 由于会话在服务端维护，在设计分布式锁时(临时节点方式)存在一个问题，A获得锁，A网络异常，B获得锁，在A网络正常后A才能获知自己丢失锁。这就存在短暂的时间内多人同时得到锁的可能性。临时序列节点也同样存在这个问题。
...
w) ZOO_CONNECTED_STATE状态存在歧义，无法获知是否新建了会话。
   状态变化介绍：
   第一次启动时              未知 ->  connected
   会话内网络短时间故障  connecting -> connected
   会话超时到重新建立会话 connecting -> expired ->connected
   CLOSED_STATE库未定义，但是库内代码使用0.
x) zk为强一致性的，所以不要用来处理要求性能的场合。

----------------------------------华丽分割线----------------------------------------------
3:本程序（如下称为sdk）实现的功能：
a) 会话失效后重建，以及订阅和临时节点的恢复。由sessionWatcher回调线程中完成。session_expired开启会话重建，session_connected事件重新订阅和临时节点恢复。
b) 支持持续订阅。
   一次订阅，持续收到变化通知。
   两次会话之间（老会话超时，新会话未建立）数据变化通知不丢失。不可依赖原始的通知机制，故采用版本号(数据内容判断效率太低)作为数据变化推送的依据。保证即没有无效通知，也不会丢失通知。
   当然两次会话之间多次数据变化，只能处理最后一次变化的通知。
c) 重建会话时和持续订阅中 的失败操作watch和 tmpnode 均保存在队列中，由单独线程定时再执行。
d) 服务注册功能：
...
5:注意点
a) 当某一个服务提供多个接口时，如{"http":"", "ws":"", "rep":""}。如果多个接口提供不同业务服务，无问题。
   当多个接口提供一种业务服务时，就必须考虑一致性问题。当使用者要与该服务建立多个连接时，务必要让这些连接练到一个服务上。当其中一条连接出现问题时，应该默认所有连接均出问题，一同切换到另一台服务器上。
b) 订阅父节点个数变化，但子节点数据变化，无法感知。服务迁移时(临时节点数据有变化，一般是ip变化)，若此时会话异常，可能无法感知。正确做法是，通知子节点有变化，但是获取的子节点后发现无任何增删，此时应该重新加载所有子节点。
c) watch回调不应该阻塞太久。
